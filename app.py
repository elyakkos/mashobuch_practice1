...
import pickle
import numpy as np
import pandas as pd # Добавляем pandas для работы с DataFrame

app = Flask(__name__)

# --- Загрузка модели и стандартизатора ---
try:
    with open('BestWineQualityModel.pkl', 'rb') as model_file:
        model = pickle.load(model_file)
    with open('scaler.pkl', 'rb') as scaler_file:
        scaler = pickle.load(scaler_file)
    print("Модель и стандартизатор успешно загружены.")
except FileNotFoundError:
    print("Ошибка: Убедитесь, что 'BestWineQualityModel.pkl' и 'scaler.pkl' находятся в той же директории, что и app.py")
    model = None
    scaler = None
...
if __name__ == '__main__':
    ...
    # debug=True: включает режим отладки (автоперезагрузка, подробные ошибки) - только для разработки!
    # host='0.0.0.0': сервер будет доступен извне (не только с localhost)
